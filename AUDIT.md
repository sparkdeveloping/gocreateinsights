# GoCreate Insights — v5.8 data and reporting audit
...
## v5.8 Modified Report integrity rule
...
## v5.8 expired-status presentation rule
...
## v5.8 Modified Report presentation audit

Repeated internal/manual-disclosure banners were removed from the report UI. Transparency is consolidated into one optional **Report details** control. The printed report remains visibly titled **Modified Report**, while the configured 84 Assistance and 62 Small Business values are labeled as report values for the fixed Sep. 1, 2025 → data-as-of period. They do not mutate imported data or source-derived analytics.

The Modified Report now carries the same high-value source-derived context expected from the Koch Report: membership mix, demographics, Assistance questionnaire categories, enrollment-reference signals, application/matching coverage, city/affiliation distributions, tracker/manual attendance, guests, review queues and historical context. Expired aggregate counts are omitted from presentation.
