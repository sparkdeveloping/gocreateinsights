...
export default function KochReport({ bootstrap, members, applications, range, openExplore, onPrint, onShowAllAssistance }: Props) {
  ...
  const outOfRangeBusiness = Math.max(0, bootstrap.overview.smallBusinessReferences - business.length);
  const outOfRangeNonprofits = Math.max(0, bootstrap.overview.nonprofitReferences - nonprofits.length);
  const outOfRangeReduced = Math.max(0, bootstrap.overview.reducedRateReferences - reduced.length);
    ...
    <div className="tab-stack koch-report-shell">
      ...
      <section className="report-page report-page-cover">
        ...
        <div className="report-metric-grid">
          ...
          <ReportMetric label="Quilters / reduced rate" value={reduced.length} note={outOfRangeReduced ? `${reduced.length} in range · ${bootstrap.overview.reducedRateReferences} all-time` : "Explicit references detected in enrollment info"} tone="yellow" onClick={() => openExplore(q("reduced-rate-reference", "Quilters / reduced-rate references", "Applications containing explicit quilter or reduced-rate language."), "applications")} />
          <ReportMetric label="Membership assistance" value={assistancePeople} note={outOfRangeAssistance ? `${assistanceApps.length} in range · ${bootstrap.overview.assistanceRequests} all-time` : `${assistanceApps.length} assistance application ${assistanceApps.length === 1 ? "row" : "rows"}`} tone="yellow" onClick={() => openExplore(q("assistance", "Membership assistance", "Applications using the membership-assistance pathway in the selected date range."), "applications")} />
          <ReportMetric label="Small-business references" value={business.length} note={outOfRangeBusiness ? `${business.length} in range · ${bootstrap.overview.smallBusinessReferences} all-time` : "Detected from enrollment/application questionnaire text"} onClick={() => openExplore(q("business-reference", "Small-business references", "Application rows with explicit small-business, startup, entrepreneur or prototype signals."), "applications")} />
          <ReportMetric label="Nonprofit references" value={nonprofits.length} note={outOfRangeNonprofits ? `${nonprofits.length} in range · ${bootstrap.overview.nonprofitReferences} all-time` : "Named or categorized organization references"} tone="ink" onClick={() => openExplore(q("nonprofit-reference", "Nonprofit / organization references", "Application rows with explicit nonprofit, church, club, 4-H, volunteering or organization signals."), "applications")} />
        </div>
      ...
      <section className="report-page">
        ...
        {outOfRangeAssistance > 0 && <button className="report-range-note" onClick={onShowAllAssistance}><InfoIcon /><span><strong>{outOfRangeAssistance} assistance row{outOfRangeAssistance === 1 ? " is" : "s are"} outside this report scope.</strong> The latest assistance application in this workbook is {formatDate(bootstrap.meta.assistanceSubmissionMax)}; the workbook contains {bootstrap.overview.assistanceRequests} assistance rows across {bootstrap.overview.assistancePeople} people, with questionnaire detail on {bootstrap.meta.assistanceQuestionnaireApplications}. Select this notice to switch to all-time assistance records.</span><ArrowUpRightIcon /></button>}

        <div className="report-two-column">
          <section className="report-chart-card">
            <div className="report-section-title"><span>Reason for assistance request</span><small>Derived from the membership-assistance questionnaire; raw free text stays private</small></div>
            {assistanceReasons.length && assistanceApps.length ? <div className="report-reason-list">{assistanceReasons.map((row, index) => <button key={row.name} onClick={() => row.name === "Reason not captured" ? openExplore(q("assistance", "Assistance applications without reason detail", "Assistance applications where a reason response is not present in the current extract."), "applications") : openExplore(q("assistance-reason", row.name, `Membership-assistance applications categorized as “${row.name}”.`, row.name), "applications")}><i style={{ background: COLORS[index % COLORS.length] }} /><span>{row.name}</span><strong>{row.value}</strong></button>)}</div> : <div className="report-empty large"><InfoIcon /><div><strong>No assistance applications fall inside this selected range.</strong><span>The latest workbook does contain {bootstrap.overview.assistanceRequests} all-time assistance rows and {bootstrap.meta.assistanceQuestionnaireResponses} questionnaire responses. Use the all-time assistance notice above to inspect their reason categories.</span></div></div>}
          </section>
          <section className="report-chart-card">
      ...
      <section className="report-page">
        ...
        <div className="report-data-note"><InfoIcon /><div><strong>How these references are counted</strong><span>The latest workbook includes the membership-assistance questionnaire in RawControls. The generator scans only those questionnaire responses and selected enrollment fields for explicit business, nonprofit/organization, quilting/reduced-rate, prototype and community-use language. Raw response text is not sent to the browser. A zero in this date range means no matching application was submitted inside the range; all-time context is shown above when older matches exist.</span></div></div>
